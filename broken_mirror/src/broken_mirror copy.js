  ...
  constructor(options){
    if(!options || !options.element) {
      throw 'argument is invalid.'
		...
		// this.src = this.options.element.getAttribute("src")
    ...
    switch(this.options.event_mode){
      case "click":
        this.options.element.addEventListener("click", this.view.bind(this))
        break
	...
	scroll(){
		const elm = this.options.element
    ...
    && rect.top - this.page_height < -100){
      // this.view()
			setTimeout(this.view.bind(this), this.option.delay)
    }
    ...
    this.frameElm                = document.createElement('div')
		this.frameElm.className      = this.options.element.className
    this.frameElm.style.position = `absolute`
    // this.frameElm.style.zIndex   = `${this.option.zIndex}`
    // this.frameElm.style.left     = `${rect.left}px`
    // this.frameElm.style.top      = `${rect.top}px`
    ...
    // 枠をbodyに追加
    this.options.element.parentNode.appendChild(this.frameElm)
  
    // ポリゴングループを作成する
    this.groups = this.getPolygonGroups(this.option, this.size)
    ...
    this.groups.forEach((group, i) => {
      group.forEach((polygon, j) => {
        ...
        imgElm.src = imgSrc
        imgElm.className             = this.options.element.className
		...
		this.groups.forEach((group, i) => {
	 		group.forEach((polygon, j) => {
	 			...
	 			if(this.frame >= q.late) {
	 				...
	 				q.x += q.vx
	 				q.y = q.vy * time + 0.5 * this.option.accelerationRate * (this.size.height / 480) * time * time
	 				q.r += q.vr
	 			}		 			
			...
			switch(this.option.event_mode){
				...
				default:
					if(this.option.finished) {
						if(this.options.loop_time){
							setTimeout(this.finish.bind(this), this.options.loop_time)
							setTimeout(this.event_mode.bind(this), this.options.loop_time + this.option.delay)
						}
						else{
							this.finish()
						}
